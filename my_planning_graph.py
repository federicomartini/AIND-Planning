...
class PlanningGraph():
	...
	def add_action_level(self, level):
		...
		# Initialize the action levels
		self.a_levels.append(set())
		
		# For all possible actions
		for action in self.all_actions:
			# Create the PgNode object
			a_node = PgNode_a(action)
			# If a_node's prenodes is a subset of the previous level
			if a_node.prenodes.issubset(self.s_levels[level]):
				# Iterate over all the nodes of the previous level and connect action level to state level
	...
	def add_literal_level(self, level):
		...
		# Get the effect nodes of the previous action
			for effect_node in a_node.effnodes:
				# Add the effect nodes to the level state nodes
	...
	def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		# For each effect ADD in node_a1
		for effect in node_a1.action.effect_add:
			# Check that it's not a REM effect for node_a2
			if effect in node_a2.action.effect_rem:
				return True

		# For each effect ADD in node_a2
		for effect in node_a2.action.effect_add:
			# Check that it's not a REM effect for node_a1
	...
	def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		for effect in node_a1.action.effect_add:
			# Check that it's not a negative precondition for node_a2
			if effect in node_a2.action.precond_neg:
				return True
		
		# For each ADD effect of node_a2
		for effect in node_a2.action.effect_add:
			# Check that it's not a negative precondition for node_a1
			if effect in node_a1.action.precond_neg:
				return True

		# For each REM effect of node_a1
		for effect in node_a1.action.effect_rem:
			# Check that it's not a positive precondition for node_a2
			if effect in node_a2.action.precond_pos:
				return True
		
		# For each REM effect of node_a2
		for effect in node_a2.action.effect_rem:
			# Check that it's not a positive precondition for node_a1
	...
	def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		# Check if preconditions of a1 and a2 are mutually exclusive
	...
	def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
		...
		# Check if the states represent the same symbol
		if node_s1.symbol == node_s2.symbol:
			is_symbol = True
		else:
			is_symbol = False
		
		# Check if the states are one the opposite of the other
		if node_s1.is_pos != node_s2.is_pos:
			is_opposite = True
		else:
			is_opposite = False
		
		return is_symbol and is_opposite
	...
	def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
		...
		# For each node s1 precondition check if the it's 
		# mutually exclusive with one of the node s2 preconditions
